...
page_free(struct PageInfo *pp)
{
	...
	// pp->pp_link is not NULL.
        ...
        if(pp->pp_ref != 0)
            panic("page_free: Nonzero pp_ref!");
        if(pp->pp_link != NULL)
            panic("page_free: Nonnull pp_link!");
...
{
    struct PageInfo *pgt_info;
    pte_t *pgtable;
    uintptr_t dir_i = PDX(va);
    uintptr_t table_i = PTX(va);
    pde_t *direntry = (pgdir+dir_i);

    if (*direntry & PTE_P){
        pgtable = (pte_t *)KADDR(PTE_ADDR(*direntry));
    }
    else{
        if (create) {
            pgt_info = page_alloc(ALLOC_ZERO);
            if(!pgt_info)
                return NULL; // Could not allocate page, no memory
            pgt_info->pp_ref += 1;
            *direntry = page2pa(pgt_info) | PTE_P | PTE_W | PTE_U;
            pgtable = (pte_t *)page2kva(pgt_info);
        }
        else 
            return NULL;
    }

    return pgtable+table_i;
}
...
{
        pte_t *pageaddr = pgdir_walk(pgdir, va, false);
        if (!pageaddr)
            return NULL; // No page mapped here

        if (pte_store)
            *pte_store = pageaddr;

        struct PageInfo * page = pa2page((physaddr_t) *pageaddr);
        return page;
}
